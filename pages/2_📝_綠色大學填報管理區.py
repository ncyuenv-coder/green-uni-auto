...
# ==========================================
# 🌟 Word 報告產製引擎 (公文級排版 + 實體 Enter 換行 + 跨單位支援)
# ==========================================
def set_run_font(run):
    run.font.name = 'Times New Roman'
    run._element.rPr.rFonts.set(qn('w:eastAsia'), '標楷體')

def generate_word_report(q_id, q_title, info_strings, desc_text, req_text, report_text, enriched_files):
    doc = Document()
...
with tab_missing:
    with st.spinner("⏳ 正在比對資料庫，找出未申報名單..."):
        ...
        if df_req.empty:
            ...
            df_req_clean = df_req_clean[df_req_clean['權責單位'] != '']
            df_req_clean = df_req_clean.drop_duplicates()
        ...
        if view_item != "請選擇...":
            ...
            for idx, row in df_q_records.iterrows():
                ...
                for i in range(1, 11):
                    ...
                    if fid:
                        # 複數單位時，圖片說明加上單位前綴
                        display_desc = f"[{u}] {desc}" if unit_count > 1 else desc
                        combined_files.append({'id': fid, 'desc': display_desc})
            ...
            with st.spinner("⏳ 正在為您產生 Word 報告檔案..."):
                docx_bytes = generate_word_report(
                    ...
                    v_desc_text, combined_reqs, combined_texts, enriched_files
                )
